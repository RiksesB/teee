...
- Each user has their own module tracking in their session (no global module variable)
- Automatic session cleanup runs every 30 minutes (removes sessions inactive for >1 hour)

### Session Lifecycle

**Session Helpers**:
- `obtenerSesionUsuario(numeroUsuario)` - Get user session with validation/logging
- `actualizarSesionUsuario(numeroUsuario, datosNuevos)` - Update session with activity timestamp
- Sessions track: `modulo`, `estado`, `respuestasDetalladas`, `resultadosModulos`, `ultimaActividad`

**User Commands**:
- `prueba` - Start/restart the course from module 1
- `salir` - Exit current test and get progress summary
- `comenzar` - Start quiz (compatibility with template button)
- Template buttons can trigger course initiation directly

### Error Code Handling

**Error 131047** (24-hour window violation):
- Automatically retries with template message if initial send fails
- Use `usarPlantilla: true` in POST endpoints to bypass 24-hour window
- Template messages allow contact outside conversation window

### Video Tutorial System

Videos are sent with fallback mechanism (index.js:702-727):
- Primary: Send video via WhatsApp video message
- Fallback: Send text message with video URL if video send fails
- Does not block quiz progression - user can continue immediately