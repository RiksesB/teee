...
// Cada usuario tiene su propia sesión aislada con su estado y módulo actual
const userSessions = new Map();

// Sistema de cola de mensajes por usuario para evitar race conditions
// Esto asegura que los mensajes de cada usuario se procesen secuencialmente
const messageQueues = new Map();
const processingUsers = new Set();

// Función helper para obtener sesión de usuario con validación
function obtenerSesionUsuario(numeroUsuario) {
  const sesion = userSessions.get(numeroUsuario);
  if (sesion) {
    console.log(`🔍 Sesión activa para ${numeroUsuario}: Módulo ${sesion.modulo}, Estado: ${sesion.estado || sesion.enFormulario ? 'formulario' : sesion.enEncuesta ? 'encuesta' : 'desconocido'}`);
  }
  return sesion;
}

// Función helper para actualizar sesión de usuario
function actualizarSesionUsuario(numeroUsuario, datosNuevos) {
  const sesionActual = userSessions.get(numeroUsuario) || {};
  const sesionActualizada = {
    ...sesionActual,
    ...datosNuevos,
    ultimaActividad: new Date() // Actualizar timestamp de actividad
  };
  userSessions.set(numeroUsuario, sesionActualizada);
  console.log(`✅ Sesión actualizada para ${numeroUsuario}: Módulo ${sesionActualizada.modulo}`);
  return sesionActualizada;
}

// Función para procesar mensajes con cola (evita race conditions)
...
async function procesarMensajeConCola(numeroUsuario, handler) {
  ...
  if (processingUsers.has(numeroUsuario)) {
    console.log(`⏳ Mensaje encolado para ${numeroUsuario} (${messageQueues.get(numeroUsuario).length} en cola)`);
    return;
  }

  // Marcar usuario como en procesamiento
  processingUsers.add(numeroUsuario);
  console.log(`🔄 Iniciando procesamiento de cola para ${numeroUsuario}`);
  ...
  console.log(`✅ Cola procesada completamente para ${numeroUsuario}`);
}
...
setInterval(() => {
  ...
  userSessions.forEach((sesion, numeroUsuario) => {
    const ultimaActividad = sesion.ultimaActividad || sesion.iniciadoEn;
    if (ultimaActividad && (ahora - ultimaActividad) > tiempoInactividadMax) {
      userSessions.delete(numeroUsuario);
      sesionesEliminadas++;
      console.log(`🗑️ Sesión inactiva eliminada para ${numeroUsuario}`);
    }
  });

  if (sesionesEliminadas > 0) {
    console.log(`🧹 Limpieza de sesiones: ${sesionesEliminadas} sesiones eliminadas`);
  }
    ...
    preguntas: [
      {
        numero: 1,
        pregunta:
          "¿Qué detalle te puede ayudar a descubrir que un correo es falso?",
        opciones: [
        ...
        retroalimentacion: {
          A: "❌ Correcto parcialmente, pero no es el único indicio. Los errores de redacción son una señal, pero los estafadores también pueden usar logos bonitos y saludos amigables.",
          B: "❌ Los estafadores no solo copian logos y colores para parecer reales, también usan otros trucos.",
          C: "❌ Un saludo amistoso no es el único indicio de inseguridad.",
          D: "✅ ¡Exacto! Los errores de ortografía, logos bonitos y saludos amigables pueden ser señales de un correo falso. Los estafadores combinan múltiples tácticas para engañarte.",
        },
      },
      {
        numero: 2,
...
// REMOVIDO: Variable global MODULO_ACTUAL causaba problemas de concurrencia
// Ahora cada usuario tiene su propio módulo en su sesión
...
async function iniciarPruebaDirecta(numeroUsuario, usarPlantilla = false) {
  ...
  if (userSessions.has(numeroUsuario)) {
    ...
    console.log(`🧹 Limpiando sesión anterior para ${numeroUsuario} (estaba en módulo ${sesionAnterior?.modulo || 'desconocido'})`);
    userSessions.delete(numeroUsuario);
  }
  ...
  console.log(`🆕 Nueva sesión creada para ${numeroUsuario}: Módulo 1`);
  ...
  try {
    ...
    try {
      await enviarVideoTutorial(numeroUsuario, sessionData.modulo);
    } catch (videoError) {
...
// Función para randomizar opciones de una pregunta (anti-copia)
function randomizarOpciones(pregunta, quitarUnaIncorrecta = false) {
  ...
  if (pregunta.opciones.length === 4 && quitarUnaIncorrecta && Math.random() > 0.5) {
    ...
    console.log(`🎲 Pregunta de 4 opciones: Quitada opción ${String.fromCharCode(65 + indiceAQuitar)} incorrecta`);
  }
  ...
  const mapeoRespuestas = {}; // letra mostrada → letra original
  const mapeoRetroalimentacion = {}; // letra mostrada → retroalimentación
  ...
  return {
    ...
    respuestaCorrectaNueva: letraCorrectaNueva
  };
}

// Función helper para enviar pregunta (decide automáticamente entre botones o lista)
async function enviarPregunta(numeroUsuario, indicePregunta) {
  const sesion = obtenerSesionUsuario(numeroUsuario);
  if (!sesion || !sesion.modulo) {
    console.error(`No hay sesión activa para usuario ${numeroUsuario}`);
    return;
  }
  ...
  const randomizado = randomizarOpciones(preguntaOriginal, true);
  ...
  console.log(`🎲 Opciones randomizadas para ${numeroUsuario}: ${numOpciones} opciones, correcta: ${randomizado.respuestaCorrectaNueva}`);

  // Si hay más de 3 opciones, usar lista. Sino, usar botones
  if (numOpciones > 3) {
    console.log(`📝 Enviando pregunta con lista (${numOpciones} opciones) para ${numeroUsuario}`);
    await enviarPreguntaConListaCustom(numeroUsuario, indicePregunta, preguntaParaMostrar);
  } else {
    console.log(`📝 Enviando pregunta con botones (${numOpciones} opciones) para ${numeroUsuario}`);
    await enviarPreguntaConBotonesCustom(numeroUsuario, indicePregunta, preguntaParaMostrar);
  }
}
...
async function iniciarFormulario(numeroUsuario) {
  try {
    ...
    const sesionAnterior = obtenerSesionUsuario(numeroUsuario);
    const moduloUsuario = sesionAnterior?.modulo || 1;

    // Validar que el módulo actual existe
    if (!modulos[moduloUsuario]) {
      console.error(`Módulo ${moduloUsuario} no existe para usuario ${numeroUsuario}`);
      return;
    }

    const nuevaSesion = actualizarSesionUsuario(numeroUsuario, {
      enFormulario: true,
      preguntaActual: 0,
    ...
    await enviarMensaje(numeroUsuario, modulos[moduloUsuario].titulo);

    // Pequeña pausa antes de enviar la primera pregunta
    setTimeout(async () => {
      try {
        await enviarPregunta(numeroUsuario, 0);
      } catch (error) {
        console.error(
...
// Función para enviar pregunta con lista - versión con opciones customizadas (randomizadas)
async function enviarPreguntaConListaCustom(numeroUsuario, indicePregunta, preguntaCustom) {
  try {
    ...
    console.log(`✅ Lista enviada con ${rows.length} opciones para usuario ${numeroUsuario}`);
  } catch (error) {
...
// Función para enviar pregunta con lista (para 4+ opciones) - DEPRECATED, usar Custom
async function enviarPreguntaConLista(numeroUsuario, indicePregunta) {
  try {
    ...
    try {
      const sesion = obtenerSesionUsuario(numeroUsuario);
      const moduloUsuario = sesion?.modulo || 1;
      const pregunta = modulos[moduloUsuario].preguntas[indicePregunta];
...
// Función para enviar pregunta con botones - versión con opciones customizadas (randomizadas)
async function enviarPreguntaConBotonesCustom(numeroUsuario, indicePregunta, preguntaCustom) {
  try {
...
// Función para enviar pregunta con botones simples (solo letras) - DEPRECATED, usar Custom
async function enviarPreguntaConBotones(numeroUsuario, indicePregunta) {
  try {
    // Obtener el módulo del usuario desde su sesión
    const sesion = obtenerSesionUsuario(numeroUsuario);
    if (!sesion || !sesion.modulo) {
      console.error(`No hay sesión activa para usuario ${numeroUsuario}`);
      return;
    ...
    if (!modulos[moduloUsuario].preguntas[indicePregunta]) {
      console.error(
        `Pregunta ${indicePregunta} no existe en módulo ${moduloUsuario} para usuario ${numeroUsuario}`
      );
      return;
    }
...
async function procesarRespuestaFormulario(numeroUsuario, message) {
  ...
  if (message.type === "interactive") {
    ...
    // Respuesta de lista (para preguntas con 4+ opciones)
      ...
      if (listId.startsWith("respuesta_")) {
        ...
        respuestaSeleccionada = letra.toUpperCase(); // Convertir a mayúscula
        console.log(`📋 Respuesta de lista recibida: ${respuestaSeleccionada}`);
      }
    }
  ...
  const moduloUsuario = sesion.modulo;
  const preguntaOriginal = modulos[moduloUsuario].preguntas[sesion.preguntaActual];

  // Obtener mapeo de opciones randomizadas para esta pregunta
  const mapeo = sesion.mapeosPreguntas?.[sesion.preguntaActual];
  ...
  if (mapeo) {
    ...
    console.log(`🎲 Mapeo usado: ${respuestaSeleccionada} → ${respuestaOriginal} original`);
  } else {
    ...
    console.log(`⚠️ Sin mapeo, usando respuesta directa`);
  }
  ...
  console.log(`📝 Usuario ${numeroUsuario} - Módulo ${moduloUsuario}, Pregunta ${sesion.preguntaActual + 1}: Respuesta ${respuestaSeleccionada} (${respuestaCorrecta ? '✅ Correcta' : '❌ Incorrecta'})`);
  ...
  if (sesion.preguntaActual < modulos[moduloUsuario].preguntas.length) {
    ...
    setTimeout(async () => {
      try {
        await enviarPregunta(numeroUsuario, sesion.preguntaActual);
      } catch (error) {
        console.error(
    ...
    const resultadoModulo = {
      ...
      porcentaje:
        (sesion.respuestasCorrectas / modulos[moduloUsuario].preguntas.length) *
        100,
    };
    ...
    await guardarRespuestasFormulario(numeroUsuario, moduloUsuario, {
      resultadoModulo,
    ...
    if (moduloUsuario < 6) {
      ...
      actualizarSesionUsuario(numeroUsuario, {
        ...sesion,
      ...
      setTimeout(async () => {
        try {
          ...
          await enviarVideoTutorial(numeroUsuario, siguienteModulo);
...
function generarResultadoModulo(respuestasCorrectas, totalPreguntas, moduloUsuario) {
  const porcentaje = (respuestasCorrectas / totalPreguntas) * 100;

  let mensaje = `🎉 ¡${modulos[moduloUsuario].titulo} completado!\n\n`;
...
async function procesarRespuestaEncuesta(numeroUsuario, message) {
  ...
  if (sesion.preguntaEncuestaActual < preguntasEncuesta.length) {
    ...
    const datosCompletos = obtenerSesionUsuario(numeroUsuario);
    ...
    userSessions.delete(numeroUsuario);
    // Ya no necesitamos resetear MODULO_ACTUAL porque cada usuario tiene su propio módulo
...
app.post("/webhook", async (req, res) => {
  ...
  if (
    ...
    // Esto garantiza que los mensajes del mismo usuario se procesen secuencialmente
...
// Función separada para procesar mensaje de usuario (llamada desde la cola)
async function procesarMensajeUsuario(numeroUsuario, message) {
  let textoMensaje = "";
  ...
  // Verificar si el usuario quiere salir
    ...
    if (
      ...
      // Módulo se resetea automáticamente al crear nueva sesión
    } else {
  ...
  // Verificar si el usuario está en un formulario activo
    ...
    obtenerSesionUsuario(numeroUsuario)?.enFormulario
  ) {
    respuesta = await procesarRespuestaFormulario(numeroUsuario, message);
  }
  // Verificar si el usuario está en una encuesta activa
    ...
    userSessions.has(numeroUsuario) &&
    obtenerSesionUsuario(numeroUsuario)?.enEncuesta
  ) {
    respuesta = await procesarRespuestaEncuesta(numeroUsuario, message);
  }
...
app.listen(PORT, () => {
  ...
  console.log("\n🔄 Sistema de limpieza de sesiones activo (cada 30 minutos)");
  console.log("📊 Las sesiones inactivas por más de 1 hora serán eliminadas automáticamente");
});
